    ...
    <div className="min-h-screen bg-zinc-50 p-4 dark:bg-black">
      <main className="mx-auto max-w-3xl space-y-6">
        ...
        <div className="space-y-3 rounded bg-white p-4 dark:bg-zinc-900">
          <SkillCard title="Fire Mastery" unlocked={skills.fireMastery} description="Fire decays slower each turn." />
          <SkillCard title="Hunting" unlocked={skills.hunting} description="Higher hunt success and more food from hunts." />
          <SkillCard title="Exploration" unlocked={skills.exploration} description="Find more positive random events while exploring." />
        </div>
      </main>
    </div>
  );
...
function SkillCard({ title, unlocked, description }: { title: string; unlocked: boolean; description: string }) {
  return (
    <div className="flex items-center justify-between rounded border border-zinc-100 p-3 dark:border-zinc-700">
      ...
      <div className="text-sm font-medium">{unlocked ? "Unlocked" : "Locked"}</div>
    </div>
  );
}
